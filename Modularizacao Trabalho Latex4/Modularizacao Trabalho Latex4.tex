...
\usetikzlibrary{calc}
\usepackage{hyperref}
\usepackage{amssymb}  % Estrela bonita
...
\newcommand{\PerfectBuildSeal}{%
\begin{tikzpicture}[scale=1.3]
    % Círculo externo
    \draw[line width=1pt] (0,0) circle (2cm);
    % Círculo interno
    \draw[line width=0.8pt] (0,0) circle (1.6cm);
    % Texto superior curvo
    \draw[
        decoration={
            text along path,
            text={|\small\bfseries|WENES AQUINO},
            text align=center
        },
        decorate
    ] (180:1.69cm) arc (180:0:1.69cm);
    % Texto inferior curvo
    \draw[
        decoration={
            text along path,
            text={|\small\bfseries|CONSTRUÇÃO PERFEITA},
            text align=center
        },
        decorate
    ] (180:1.90cm) arc (180:360:1.90cm); 
    % Estrela central
    \node at (0,0.8) {\large$\bigstar\bigstar\bigstar\bigstar\bigstar$};
    % Texto do meio
    \node at (0,0.3) {\bfseries\small ZERO};
    \node at (0,-0.1) {\bfseries\small WARNINGS};
    \node at (0,-0.6) {\tiny Copyright © 2026 - Wenes};
\end{tikzpicture}
}
...
\newcommand{\capituloCentralizado@nostar}[1]{%
  ...
  \chapter*{#1}%
  \markboth{#1}{}%
  \addcontentsline{toc}{chapter}{#1}%  % entra no sumário
